...
A bash script that recreates this entire configuration on a fresh Pi OS Bookworm install is available at:

```
~/.openclaw/workspace/scripts/setup-touchscreen.sh
```

Run with `sudo bash setup-touchscreen.sh` — it handles:

1. **`/boot/firmware/config.txt`** — adds `ilitek251x` overlay (GPIO interrupt pin 4), removes stale `ili210x` if present
2. **udev rule** — writes the 270° calibration matrix (`0 1 0 -1 0 1 0 0 1`)
3. **labwc autostart** — Chromium kiosk pointed at Home Assistant
4. **labwc rc.xml + environment** — input settings + US keyboard layout
5. **wayfire.ini** — display rotation via `wlr-randr --transform 270`, kiosk autostart, DPMS/screensaver disabled
6. **Packages** — installs labwc, autotouch, wlr-randr, squeekboard, chromium, lightdm

The `HASS_URL` variable at the top of the script can be changed if the Home Assistant address differs.

> **Note:** The script does NOT set up the hwdb range override (`/etc/udev/hwdb.d/99-touchscreen.hwdb`). If your touch panel has a mismatched coordinate range (like the ILI210x reporting 0–16383 but only using 31–1919), you'll still need to follow Steps 2–3 above to determine and set the correct range.
